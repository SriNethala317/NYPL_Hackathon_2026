...
export async function runEndToEndScenario() {
  ...
  const recommendations = await discoverBenefits(DEMO_NYC_STUDENT_PROFILE, { catalogProvider: new FixtureCatalogProvider() });
  ...
  const geminiRequest = buildGeminiEnhancementRequest([{ programId: selected.programId, programName: selected.programName, description: selected.summary, source: { type: 'fixture' } }], toBenefitRecommendationContext(DEMO_NYC_STUDENT_PROFILE)); assertNoPii(geminiRequest);
